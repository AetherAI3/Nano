Ÿ˜ZkŒzÂ∆πÔ«ıÁ÷ùÔmÌΩ9oû]€á—Ω˜ÛWoM<„ü5kmZoFùı≠\◊F∏˜øtkﬁús◊:<div align="center">

<img src="https://raw.githubusercontent.com/AetherAI3/Nano/main/assets/nano-hex.png" alt="Nano logo" width="210" />

# Nano

### Trading and agent rules that run the same way every time ‚Äî with a receipt.
...
### **[‚Üí Browse the Strategy Library](https://aethersystems.net/nano)**

Read every strategy in this repo ‚Äî source, compiled IR, signals and the corpus
provenance policy ‚Äî in your browser. No install, no account. Open any one of them
...
- **Deterministic** ‚Äî the same rule over the same inputs replays to the same decision, every time.
- **Auditable** ‚Äî every run carries an ordered log you can archive, diff, or hand to someone who is asking questions.
- **Host-governed** ‚Äî Nano *proposes* an intent; your system disposes. Nano cannot place an order.
- **Small** ‚Äî few moving parts to audit, and it embeds in a Python stack you already have.

**Beta reference implementation for Nano IR/language v1.0.0** ‚Äî `Development Status :: 4 - Beta`. This is the language and IR version, not the independently versioned installable package; `pyproject.toml` is authoritative for the package release. The examples are trading-oriented, but Nano fits any system where a host supplies numeric signals and must retain control over what happens next ‚Äî see [deterministic watchdogs and compliance controls](#deterministic-watchdogs-and-compliance-controls).
...
`RSI(14)` is the **feed-signal form**: the host computes and injects the `RSI` series. v1.0 adds a **computed form** ‚Äî `RSI(close, 14)`, where `close` is a declared `input` and Nano derives the series itself from 40 deterministic kernels. Nano still never *fetches* market data. See the [language reference](https://github.com/AetherAI3/Nano/blob/main/docs/language.md) for both contracts.
...
The [strategy library](https://github.com/AetherAI3/Nano/blob/main/nano/library/README.md) is Nano's community on-ramp: a small, tested corpus of familiar trading ideas ‚Äî and deterministic watchdog controls ‚Äî translated into the DSL. Every entry pairs readable `.nano` source with expected IR, so quant researchers can learn the language, compare conventions, and contribute a new rule with confidence.

**[Your first contribution ‚Üí](https://github.com/AetherAI3/Nano/blob/main/docs/first-contribution.md)** walks the whole path once, with a real rule: idea, source, generated IR, deterministic replay, CI, pull request. One command generates the IR fixture and checks the entry before you push it.

The same corpus is browsable at **[aethersystems.net/nano](https://aethersystems.net/nano)** ‚Äî search and filter by category, cadence or signal, read the source and compiled IR side by side, and open any strategy directly in an editor. Nothing to install, no account needed.
...
Strategy names and indicator names such as Bollinger Bands are used nominatively,
...
not original ‚Äî see the [library README](https://github.com/AetherAI3/Nano/blob/main/nano/library/README.md#provenance).

[Browse online ‚Üí](https://aethersystems.net/nano) ¬∑ [Browse in-repo ‚Üí](https://github.com/AetherAI3/Nano/blob/main/nano/library/README.md) ¬∑ [Your first contribution ‚Üí](https://github.com/AetherAI3/Nano/blob/main/docs/first-contribution.md) ¬∑ [Add a strategy ‚Üí](https://github.com/AetherAI3/Nano/blob/main/CONTRIBUTING.md#add-a-strategy) ¬∑ [Propose a language change ‚Üí](https://github.com/AetherAI3/Nano/issues/new?template=language-change.yml)
...
v1.0 adds static typing with `series<T>`, look-ahead protection, arithmetic and `or`/`not`, `param`/`input`/`let` declarations, computed indicators, and a CLI. It still has no LLM runtime, live data feed, or action executor ‚Äî a reasoning provider is a protocol the host implements. [`docs/status.md`](https://github.com/AetherAI3/Nano/blob/main/docs/status.md) separates implemented behavior from experimental work and future ideas.
...
Nano IR/language v1.0.0 provides the small scheduled-threshold foundation used
by these examples. Broader watchdog and compliance deployments require host
integrations, domain-specific signal contracts, durable persistence,
authorization gates, and security review.
